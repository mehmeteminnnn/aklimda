...
class _GameSetupScreenState extends State<GameSetupScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(body: SafeArea(child: GardenBackground(child: Form(key:_formKey,child:ListView(padding:const EdgeInsets.fromLTRB(20,16,20,28),children:[
      ...
      const SizedBox(height:12), _Panel(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[_Label('Kart seti', GameGlyphType.cards),const SizedBox(height:12),Wrap(spacing:8,runSpacing:8,children:GameProvider.cardSets.keys.map((name)=>ChoiceChip(label:Text(name),selected:set==name,onSelected:(_)=>context.read<GameProvider>().setCardSet(name))).toList()),const SizedBox(height:12),SizedBox(height:58,child:Row(children:GameProvider.cardSets[set]!.take(5).map((id)=>Expanded(child:Padding(padding:const EdgeInsets.symmetric(horizontal:2),child:DecoratedBox(decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(12)),child:CardFaceIllustration(symbolId:id,category:CardAssets.categoryForSet(set))))).toList()))])),
